...
Vectors in R support component-wise operations, which is best illustrated through examples.
...
Create lists using `list()`. The components of lists can also be named. Indexing lists with double square brackets. 
...
R has built-in functions for working with many probability distributions covered in STAT 110. For any family of distributions in R (Binomial, Normal, etc.), there are four functions. For example, with Normal distributions, we have:
...
R can handle various matrix operations. First, indexing can be accomplished using single square brackets. 
...
By supply elements to a new index, you can also add elements to the data frame.
...
To view the first few rows of a data frame or first few elements of a vector/list, use `head()`. To view the last elements, use `tail()`. You can supply an optional second element specifying the number of elements you want to see.  
...
The function `replicate` repeatedly evaluates an expression. This is useful for simulations. For example, if our experiment was to observe 10 i.i.d. r.v.s with an Exponential distirbution with $\lambda = 1$, we can replicate the experiment 100 times using the following:

```{r replicate_ex}
result <- replicate(100, rexp(10, 1))
```
...
U of U says that if we plug a uniform into the inverse-CDF, we will observe draws from the desired distribution. First find the inverse CDF analytically (the equation of the inverse CDF).
